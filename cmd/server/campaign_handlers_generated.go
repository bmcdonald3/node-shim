// Code generated by Fabrica dev. DO NOT EDIT.
// Template: server/handlers.go.tmpl
// Generated: 2026-03-26T11:20:49-07:00
//
// # Copyright © 2025 OpenCHAMI a Series of LF Projects, LLC
//
// SPDX-License-Identifier: MIT
//
// This file contains REST API handlers for Campaign resources.
//
// To modify this code:
//  1. Edit the template file: pkg/codegen/templates/handlers.go.tmpl
//  2. Run 'make dev' to regenerate
//  3. Do NOT edit this file directly - changes will be lost
//
// Generated handlers provide:
//   - GET /campaigns (list all campaigns)
//   - GET /campaigns/{uid} (get specific Campaign)
//   - POST /campaigns (create new Campaign)
//   - PUT /campaigns/{uid} (update Campaign spec)
//   - PATCH /campaigns/{uid} (patch Campaign spec)
//   - DELETE /campaigns/{uid} (delete Campaign)
//   - PUT /campaigns/{uid}/status (update Campaign status)
//   - PATCH /campaigns/{uid}/status (patch Campaign status)
//
// Authorization: Add custom middleware for authentication/authorization
// Storage: Uses storage.LoadCampaign*/SaveCampaign*/DeleteCampaign*
// Version Support: Available (see version context in handlers)
//
// To enable full version conversion for this resource:
//  1. Add new version: fabrica add version <group> <version>
//  2. Implement converter: apis/<group>/<version>/converter.go
//  3. Add version-aware storage: storage.LoadCampaignWithVersion()
//  4. Register versions in cmd/server/main.go
package main

import (
	"encoding/json"
	"fmt"
	"io"
	"net/http"
	"time"

	"github.com/go-chi/chi/v5"
	"github.com/openchami/fabrica/pkg/events"
	"github.com/openchami/fabrica/pkg/patch"
	"github.com/openchami/fabrica/pkg/resource"
	"github.com/openchami/fabrica/pkg/validation"
	"github.com/openchami/fabrica/pkg/versioning"
	v1 "github.com/user/node-service/apis/example.fabrica.dev/v1"

	"github.com/user/node-service/internal/storage"
)

// GetCampaigns returns all Campaign resources
func GetCampaigns(w http.ResponseWriter, r *http.Request) {
	// Authorization: Add custom middleware in routes.go or implement checks here
	// Example: if !authorized(r) { respondError(w, http.StatusUnauthorized, fmt.Errorf("unauthorized")); return }

	campaigns, err := storage.LoadAllCampaigns(r.Context())
	if err != nil {
		respondError(w, http.StatusInternalServerError, fmt.Errorf("failed to load campaigns: %w", err))
		return
	}
	respondJSON(w, http.StatusOK, campaigns)
}

// GetCampaign returns a specific Campaign resource by UID
func GetCampaign(w http.ResponseWriter, r *http.Request) {
	uid := chi.URLParam(r, "uid")
	if uid == "" {
		respondError(w, http.StatusBadRequest, fmt.Errorf("Campaign UID is required"))
		return
	}

	// Version context available here for version-aware operations
	// versionCtx := versioning.GetVersionContext(r.Context())
	// Requested version: versionCtx.ServeVersion
	// To enable: replace storage.LoadCampaign() with version-aware function

	// Authorization: Add custom middleware in routes.go or implement checks here
	// Example: if !authorized(r) { respondError(w, http.StatusUnauthorized, fmt.Errorf("unauthorized")); return }

	campaign, err := storage.LoadCampaign(r.Context(), uid)
	if err != nil {
		respondError(w, http.StatusNotFound, fmt.Errorf("Campaign not found: %w", err))
		return
	}
	respondJSON(w, http.StatusOK, campaign)
}

// CreateCampaign creates a new Campaign resource
func CreateCampaign(w http.ResponseWriter, r *http.Request) {
	var req CreateCampaignRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		respondError(w, http.StatusBadRequest, fmt.Errorf("invalid request body: %w", err))
		return
	}

	// Layer 1: Request validation (validates inline spec fields and metadata)
	if err := validation.ValidateResource(&req); err != nil {
		respondError(w, http.StatusBadRequest, fmt.Errorf("validation failed: %w", err))
		return
	}

	// Get version context from request (set by version negotiation middleware)
	versionCtx := versioning.GetVersionContext(r.Context())

	uid, err := resource.GenerateUIDForResource("Campaign")
	if err != nil {
		respondError(w, http.StatusInternalServerError, fmt.Errorf("failed to generate UID: %w", err))
		return
	}

	// Versioned mode: flat fields with fabrica.Metadata
	campaign := &v1.Campaign{
		// Use negotiated ServeVersion (from Accept header) for apiVersion
		APIVersion: versionCtx.ServeVersion,
		Kind:       "Campaign",
		Spec:       req.AsSpec(),
	}

	// Initialize metadata from request
	campaign.Metadata = req.Metadata
	campaign.Metadata.UID = uid
	now := time.Now()
	campaign.Metadata.CreatedAt = now
	campaign.Metadata.UpdatedAt = now

	// Set labels and annotations
	if campaign.Metadata.Labels == nil {
		campaign.Metadata.Labels = make(map[string]string)
	}
	for k, v := range req.Labels {
		campaign.Metadata.Labels[k] = v
	}
	if campaign.Metadata.Annotations == nil {
		campaign.Metadata.Annotations = make(map[string]string)
	}
	for k, v := range req.Annotations {
		campaign.Metadata.Annotations[k] = v
	}

	// Layer 2: Custom business logic validation
	if err := validation.ValidateWithContext(r.Context(), campaign); err != nil {
		respondError(w, http.StatusBadRequest, fmt.Errorf("validation failed: %w", err))
		return
	}

	// Set initial status
	// This assumes the generator passes an 'IsReconcilable' boolean
	// to this template, and that the resource has a .Status.Phase field.

// Save (Layer 1: Ent validation happens automatically if using Ent storage)
if err := storage.CreateCampaign(r.Context(), campaign); err != nil {
respondError(w, http.StatusInternalServerError, fmt.Errorf("failed to create Campaign: %w", err))
return
}

	// Publish resource created event

	if err := events.PublishResourceCreated(r.Context(), "Campaign", campaign.Metadata.UID, campaign.Metadata.Name, campaign); err != nil {
		// Log the error but don't fail the request - events are non-critical
		fmt.Printf("Warning: Failed to publish resource created event for Campaign %s: %v\n", campaign.Metadata.UID, err)

	}

	respondJSON(w, http.StatusCreated, campaign)
}

// UpdateCampaign updates the spec of an existing Campaign resource
// NOTE: This endpoint ONLY updates the spec. Use PUT //campaigns/{uid}/status to update status.
func UpdateCampaign(w http.ResponseWriter, r *http.Request) {
	uid := chi.URLParam(r, "uid")
	if uid == "" {
		respondError(w, http.StatusBadRequest, fmt.Errorf("Campaign UID is required"))
		return
	}

	campaign, err := storage.LoadCampaign(r.Context(), uid)
	if err != nil {
		respondError(w, http.StatusNotFound, fmt.Errorf("Campaign not found: %w", err))
		return
	}

	var req UpdateCampaignRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		respondError(w, http.StatusBadRequest, fmt.Errorf("invalid request body: %w", err))
		return
	}

	// Apply updates

	// Versioned mode: direct field access
	if req.Metadata.Name != "" {
		campaign.Metadata.Name = req.Metadata.Name
	}

	// Update spec fields ONLY - status should use /status subresource
	campaign.Spec = req.AsSpec()

	// Update labels and annotations
	if campaign.Metadata.Labels == nil {
		campaign.Metadata.Labels = make(map[string]string)
	}
	for k, v := range req.Labels {
		campaign.Metadata.Labels[k] = v
	}
	if campaign.Metadata.Annotations == nil {
		campaign.Metadata.Annotations = make(map[string]string)
	}
	for k, v := range req.Annotations {
		campaign.Metadata.Annotations[k] = v
	}

	// Update timestamp
	campaign.Metadata.UpdatedAt = time.Now()

	if err := storage.SaveCampaign(r.Context(), campaign); err != nil {
		respondError(w, http.StatusInternalServerError, fmt.Errorf("failed to save Campaign: %w", err))
		return
	}

	// Publish resource updated event
	updateMetadata := map[string]interface{}{
		"updatedAt": campaign.Metadata.UpdatedAt,
	}

	if err := events.PublishResourceUpdated(r.Context(), "Campaign", campaign.Metadata.UID, campaign.Metadata.Name, campaign, updateMetadata); err != nil {
		// Log the error but don't fail the request - events are non-critical
		fmt.Printf("Warning: Failed to publish resource updated event for Campaign %s: %v\n", campaign.Metadata.UID, err)

	}

	respondJSON(w, http.StatusOK, campaign)
}

// PatchCampaign patches an existing Campaign resource spec using JSON Merge Patch, JSON Patch, or Shorthand Patch
// Only the spec portion of the resource can be patched - metadata and status are API-managed
func PatchCampaign(w http.ResponseWriter, r *http.Request) {
	uid := chi.URLParam(r, "uid")
	if uid == "" {
		respondError(w, http.StatusBadRequest, fmt.Errorf("Campaign UID is required"))
		return
	}

	campaign, err := storage.LoadCampaign(r.Context(), uid)
	if err != nil {
		respondError(w, http.StatusNotFound, fmt.Errorf("Campaign not found: %w", err))
		return
	}

	// Read patch document
	patchData, err := io.ReadAll(r.Body)
	if err != nil {
		respondError(w, http.StatusBadRequest, fmt.Errorf("failed to read patch data: %w", err))
		return
	}

	// Marshal current spec to JSON for patching (only allow spec modifications)
	currentSpecJSON, err := json.Marshal(campaign.Spec)
	if err != nil {
		respondError(w, http.StatusInternalServerError, fmt.Errorf("failed to marshal current spec: %w", err))
		return
	}

	// Detect patch type from Content-Type header
	contentType := r.Header.Get("Content-Type")
	patchType := patch.DetectPatchType(contentType)

	// Apply patch to spec only
	patchResult, err := patch.ApplyPatchWithOptions(currentSpecJSON, patchData, patchType, patch.PatchOptions{
		AllowAddFields:    true,
		AllowRemoveFields: true,
	})
	if err != nil {
		respondError(w, http.StatusUnprocessableEntity, fmt.Errorf("failed to apply patch to spec: %w", err))
		return
	}

	// Unmarshal the patched result back to the spec
	if err := json.Unmarshal(patchResult.Updated, &campaign.Spec); err != nil {
		respondError(w, http.StatusInternalServerError, fmt.Errorf("failed to unmarshal patched spec: %w", err))
		return
	}

	// Touch to update metadata

	campaign.Metadata.UpdatedAt = time.Now()

	// Save the patched resource
	if err := storage.SaveCampaign(r.Context(), campaign); err != nil {
		respondError(w, http.StatusInternalServerError, fmt.Errorf("failed to save patched Campaign: %w", err))
		return
	}

	// Publish resource patched event
	patchMetadata := map[string]interface{}{
		"patchType": patchType,
		"updatedAt": campaign.Metadata.UpdatedAt,
	}

	if err := events.PublishResourcePatched(r.Context(), "Campaign", campaign.Metadata.UID, campaign.Metadata.Name, campaign, patchMetadata); err != nil {
		// Log the error but don't fail the request - events are non-critical
		fmt.Printf("Warning: Failed to publish resource patched event for Campaign %s: %v\n", campaign.Metadata.UID, err)

	}

	respondJSON(w, http.StatusOK, campaign)
}

// UpdateCampaignStatus updates only the status of a Campaign resource
// This endpoint is intended for controllers, reconcilers, and monitoring systems.
// It does not modify the spec or metadata (except updatedAt timestamp).
//
// Authorization: Requires 'update_status' permission (separate from 'update' permission)
// Events: Publishes resource updated event with updateType: "status"
func UpdateCampaignStatus(w http.ResponseWriter, r *http.Request) {
	uid := chi.URLParam(r, "uid")
	if uid == "" {
		respondError(w, http.StatusBadRequest, fmt.Errorf("Campaign UID is required"))
		return
	}

	// Authorization: Add custom middleware for status update authorization
	// Status updates can have different permissions than spec updates

	res, err := storage.LoadCampaign(r.Context(), uid)
	if err != nil {
		respondError(w, http.StatusNotFound, fmt.Errorf("Campaign not found: %w", err))
		return
	}

	var statusUpdate v1.CampaignStatus
	if err := json.NewDecoder(r.Body).Decode(&statusUpdate); err != nil {
		respondError(w, http.StatusBadRequest, fmt.Errorf("invalid status body: %w", err))
		return
	}

	// Preserve spec - only update status
	res.Status = statusUpdate

	res.Metadata.UpdatedAt = time.Now()

	if err := storage.SaveCampaign(r.Context(), res); err != nil {
		respondError(w, http.StatusInternalServerError, fmt.Errorf("failed to save Campaign status: %w", err))
		return
	}

	// Publish status update event
	statusMetadata := map[string]interface{}{
		"updatedAt":  res.Metadata.UpdatedAt,
		"updateType": "status",
	}

	if err := events.PublishResourceUpdated(r.Context(), "Campaign", res.Metadata.UID, res.Metadata.Name, res, statusMetadata); err != nil {
		// Log but don't fail - events are non-critical
		fmt.Printf("Warning: Failed to publish status update event for Campaign %s: %v\n", res.Metadata.UID, err)

	}

	respondJSON(w, http.StatusOK, res)
}

// PatchCampaignStatus patches only the status of a Campaign resource
// Supports JSON Merge Patch, JSON Patch, and Shorthand Patch formats.
// Only modifies status fields - spec and metadata are preserved.
func PatchCampaignStatus(w http.ResponseWriter, r *http.Request) {
	uid := chi.URLParam(r, "uid")
	if uid == "" {
		respondError(w, http.StatusBadRequest, fmt.Errorf("Campaign UID is required"))
		return
	}

	// Authorization: Add custom middleware for status patch authorization
	// Status patches can have different permissions than spec patches

	res, err := storage.LoadCampaign(r.Context(), uid)
	if err != nil {
		respondError(w, http.StatusNotFound, fmt.Errorf("Campaign not found: %w", err))
		return
	}

	patchData, err := io.ReadAll(r.Body)
	if err != nil {
		respondError(w, http.StatusBadRequest, fmt.Errorf("failed to read patch data: %w", err))
		return
	}

	// Marshal current status for patching
	currentStatusJSON, err := json.Marshal(res.Status)
	if err != nil {
		respondError(w, http.StatusInternalServerError, fmt.Errorf("failed to marshal current status: %w", err))
		return
	}

	contentType := r.Header.Get("Content-Type")
	patchType := patch.DetectPatchType(contentType)

	patchResult, err := patch.ApplyPatchWithOptions(currentStatusJSON, patchData, patchType, patch.PatchOptions{
		AllowAddFields:    true,
		AllowRemoveFields: false, // Don't allow removing status fields
	})
	if err != nil {
		respondError(w, http.StatusUnprocessableEntity, fmt.Errorf("failed to apply patch to status: %w", err))
		return
	}

	// Unmarshal patched status back
	if err := json.Unmarshal(patchResult.Updated, &res.Status); err != nil {
		respondError(w, http.StatusInternalServerError, fmt.Errorf("failed to unmarshal patched status: %w", err))
		return
	}

	res.Metadata.UpdatedAt = time.Now()

	if err := storage.SaveCampaign(r.Context(), res); err != nil {
		respondError(w, http.StatusInternalServerError, fmt.Errorf("failed to save patched Campaign status: %w", err))
		return
	}

	// Publish status patch event
	patchMetadata := map[string]interface{}{
		"patchType":  patchType,
		"updatedAt":  res.Metadata.UpdatedAt,
		"updateType": "status",
	}

	if err := events.PublishResourcePatched(r.Context(), "Campaign", res.Metadata.UID, res.Metadata.Name, res, patchMetadata); err != nil {
		fmt.Printf("Warning: Failed to publish status patch event for Campaign %s: %v\n", res.Metadata.UID, err)

	}

	respondJSON(w, http.StatusOK, res)
}

// DeleteCampaign deletes a Campaign resource
func DeleteCampaign(w http.ResponseWriter, r *http.Request) {
	uid := chi.URLParam(r, "uid")
	if uid == "" {
		respondError(w, http.StatusBadRequest, fmt.Errorf("Campaign UID is required"))
		return
	}

	// Load resource before deletion for event publishing
	campaign, err := storage.LoadCampaign(r.Context(), uid)
	if err != nil {
		respondError(w, http.StatusNotFound, fmt.Errorf("Campaign not found: %w", err))
		return
	}

	if err := storage.DeleteCampaign(r.Context(), uid); err != nil {
		respondError(w, http.StatusInternalServerError, fmt.Errorf("failed to delete Campaign: %w", err))
		return
	}

	// Publish resource deleted event
	deleteMetadata := map[string]interface{}{
		"deletedAt": time.Now(),
	}

	if err := events.PublishResourceDeleted(r.Context(), "Campaign", campaign.Metadata.UID, campaign.Metadata.Name, deleteMetadata); err != nil {
		// Log the error but don't fail the request - events are non-critical
		fmt.Printf("Warning: Failed to publish resource deleted event for Campaign %s: %v\n", campaign.Metadata.UID, err)

	}

	respondJSON(w, http.StatusOK, &DeleteResponse{
		Message: "Campaign deleted successfully",
		UID:     uid,
	})
}
